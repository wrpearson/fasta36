...
ggsearch_bin = pgm_bin+"/ggsearch36"
...
annot_cmds = {'pfam':'"!../scripts/ann_pfam30.pl+--pfacc+--split_over"'}
...
def check_args(test_args=None):
  ...
  parser = argparse.ArgumentParser(description='Iterative search with SSEARCH/PSIBLAST')
  parser.add_argument('--query', dest='query_file', action='store',help='query sequence file')
  parser.add_argument('--sequence', dest='query_file', action='store',help='query sequence file')
  parser.add_argument('--db', dest='db_file', action='store',help='sequence database name')
  parser.add_argument('--database', dest='db_file', action='store',help='sequence database name')
  parser.add_argument('--dir', dest='tmp_dir', action='store',help='directory for result and tmp_file output')
  parser.add_argument('--pssm_evalue', dest='pssm_evalue', default=0.002, type=float, action='store',
                         help='E()-value threshold for inclusion in PSSM')
  parser.add_argument('--search_evalue', dest='srch_evalue', default=5.0, type=float, action='store',
                         help='E()-value threshold for search display')
  parser.add_argument('--m_format', dest='m_format', action='store',help='input result format m8 [def] or m9')
  parser.add_argument('--annot_db', dest='annot_type', action='store',help='source of domain annotations')
  parser.add_argument('--suffix', dest='suffix', action='store',help='suffix for result output')
  parser.add_argument('--out_name', dest='file_out', action='store',help='result file name')
  parser.add_argument('--num_iter', dest='num_iter', default=5, type=int, action='store',help='number of iterations to run')
  parser.add_argument('--in_pssm', dest='prev_pssm', action='store',help='initial PSSM')
  parser.add_argument('--in_bounds', dest='prev_bound_in', type=str, action='store',help='initial boundaries')
  parser.add_argument('--domain', dest='dom_flag', action='store_true',help='use domain annotations')
  parser.add_argument('--align', dest='align_flag', default=0, action='store_true',help='use alignment boundaries')
  parser.add_argument('--pgm', dest='srch_pgm', action='store',default='ssearch',help='search program: ssearch/psiblast')
  parser.add_argument('--query_seed', dest='query_mask', action='store_true',help='use query seeding')
  parser.add_argument('--prev_m89res', dest='prev_m89res', action='store', help='prevous iteration result')
  parser.add_argument('--prev_msa', dest='prev_msa', action='store', help='prevous MSA')
  parser.add_argument('--sel_res', dest='prev_sel_res', action='store', help='selected accession file')
  parser.add_argument('--this_iter', dest='this_iter', help='this iteration number',type=int)
  parser.add_argument('--int_seed', dest='int_mask', action='store',default='none',help='sequence masking: none/query/random')
  parser.add_argument('--end_seed', dest='end_mask', action='store',default='none',help='sequence masking: none/query/random')
  parser.add_argument('--int_mask', dest='int_mask', action='store',default='none',help='sequence masking: none/query/random')
  parser.add_argument('--end_mask', dest='end_mask', action='store',default='none',help='sequence masking: none/query/random')
  parser.add_argument('--save_list', dest='tmp_file_list', action='store',help='temporary extensions saved')
  parser.add_argument('--save_all', dest='save_all', action='store_true',help='save all temporary files')
  parser.add_argument('--delete_all', dest='delete_tmp', action='store_true',help='delete all temporary files')
  parser.add_argument('--delete_bnd', dest='delete_bnd', action='store_true',help='delete boundary temporary file')
  parser.add_argument('--use_stdout', dest='use_stdout', action='store_true',help='send results to stdout',default=False)
  parser.add_argument('--errors', dest='error_log', action='store_true', help='log errors', default=False)
  parser.add_argument('--quiet', dest='quiet', action='store_true',help='fewer messages')
  parser.add_argument('-Q', dest='quiet', action='store_true',help='fewer messages')
...
# builds an ssearch command line with query, db, and pssm
#
def get_ggsearch_cmd(query_file, db_file, pssm_file, args) :

  search_cmd = '%s -S -m 8CB -d 0 -E "%f 0" -s BP62' % (ggsearch_bin, args.srch_evalue)
...
#
def build_msa_pssm(query_file, this_file_out,prev_bound_in, args) :
  ...
  if (args.prev_sel_res) :
    aln2msa_cmd += " --sel_res %s" % (args.prev_sel_res)
  else:
  ...
  log_system("%s %s > %s"%(aln2msa_cmd, this_file_out, this_msa), args.error_log)

  makeblastdb_cmd = "%s -in %s -dbtype prot -parse_seqids > %s" % (makeblastdb_bin, this_hit_db, blastdb_err)

  log_system(makeblastdb_cmd, args.error_log)
  ...
  log_system("%s > %s 2> %s.err" % (buildpssm_cmd, this_psibl_out, this_psibl_out), args.error_log)

  if not (args.save_all):
    log_system("rm %s.p* %s" % (this_hit_db,blastdb_err), args.error_log)

  # remove uninformative error logs
  if (not args.error_log) :
    log_system("rm "+this_psibl_out+".err ",args.error_log)

  if (srch_pgm != 'psiblast') :
      asn2asn_cmd = "%s -v %s -e %s" % (datatool_bin, this_pssm_asntxt, this_pssm_asnbin)
      log_system(asn2asn_cmd, args.error_log)
...
def main():
  ...
  #### this section deals with maintaining intermediate files
  ...
  while (it < args.num_iter) :
    ...
    (this_pssm, this_bound_out) = build_msa_pssm(args.query_file, this_file_out, prev_bound_in, args)
