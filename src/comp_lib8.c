...
/*  $Id: comp_lib8.c 1075 2012-11-13 15:39:55Z wrp $ */
/*  $Revision: 1075 $  */
...
void print_sum(FILE *, struct db_str *qtt, struct db_str *ntt, int, long);
...
main (int argc, char *argv[]) 
{
  ...
  *aa1save=0;
  ...
  if (m_msg.lname[0] == '!') {
    if ((lib_db_file = build_lib_db(&m_msg.lname[1]))==NULL) {
      fprintf(stderr,"***[comp_lib8.c] Cannot open/execute %s script\n",&m_msg.lname[1]);
      exit(1);
    }
  ...
  */
  ...
  if (m_msg.lname[0] == '!' && !pst.debug_lib) unlink(lib_db_file);

  tdone = s_time();

#ifdef DEBUG
  fprintf(stderr,"[*** comp_lib8.c***] seqr_chains: %d [%d]; aa1b_buffer: %ld [%ld] blocks (lost: %ld); using memory:%d\n",
