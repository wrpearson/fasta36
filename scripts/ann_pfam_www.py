...
# ann_pfam_www.py gets an annotation file from fasta36 -V with a line of the form:
...
(auto_reg,rpd2_fams, neg_doms, vdoms, lav, no_clans, pf_acc_flag, shelp, help, no_over, acc_comment, bound_comment, pfamB) =
...
show_color = 1
(min_nodom, min_vdom) = (10, 10)

color_sep_str = " :"
color_sep_str = '~'

def show_annots(query_len, get_annot_sub):

    seq_len = 0
    (annot_line, seq_len) = query_len.split("\t")

    annot_data = {seq_info=>$annot_line}

    use_acc = 1

    if (re.search(r'^gi\|',annot_line)):
        (tmp, gi, sdb, acc, id) = annot_line.split('|')

    elif (re.search(r'^(sp|tr|up)\|', annot_line)):
        (sdb, acc, id) = annot_line.split('|')

    else:
      acc = re.split(r'\s',annot_line).group(1)
...
def get_pfam_www( acc, seq_length):
    ...
    json_info = json.loads(pfam_info)

    pf_domains = json_info['results']


    pf_domains.sort(key=lambda x : x['
...
def domain_name(value, seq_id, pf_acc):

    is_virtual = False

    if (re.search(r'^@',value)):
        is_virtual = True
        value = re.subs(r'^@','',value)


    if (not value):
        sys.stderr.write("missing domain name for %s\n",(seq_id))
        return ''
...
## currently, clan information is missing from the EBI interpro/api
##
##    "no-clans" => \$no_clans,
##    "no_clans" => \$no_clans,
...
def main():
    ...
    parser.add_argument('--pfacc',dest='pf_acc_flag',action='store_true',default=False)
    parser.add_argument('--pfam_acc',dest='pf_acc_flag',action='store_true',default=False)
